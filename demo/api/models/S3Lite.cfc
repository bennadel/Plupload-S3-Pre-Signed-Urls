component
	...
	*/
	...
	/**
	* I generate a pre-signed / pre-authenticated URL to GET the given resource.
	* 
	...
	*/
	...
	/**
	* I generate a pre-signed / pre-authenticated URL to PUT (ie, upload) the given resource 
	* to Amazon S3. Like a GET-based pre-signed URL, this will only be valid until the expiration
	* date, at which time access will be denied.
	* 
	* @key I am the S3 object key (does not start with "/").
	* @contentType I am the content-type of the uploaded file.
	* @expiresAt I am the expiration date of the URL (assumed to be UTC).
	* @output false
	*/
	public string function getPreSignedUrlForUpload(
		required string key,
